import { Tile, Hand, MahjongMessage, action } from "../../common/types";
import { CheckMahjong } from "../../common/utils";
...
export class Player {
    ...
    */
    ...
    //other than mahjong
    HasAction(discardedTile: Tile,discardingPlayer: number): Array<action> {
        const possiblePungs: action = {
          action: MahjongMessage.PUNG,
          interactedTiles: this.HasPung(discardedTile),
        }
        const possibleKongs: action = {
          action: MahjongMessage.KONG,
          interactedTiles: this.HasKong(discardedTile),
        }
        const possibleChows: action = {
          action: MahjongMessage.CHOW,
          interactedTiles: this.HasChow(discardedTile,discardingPlayer),
        }
        return [possiblePungs,possibleKongs,possibleChows];
    }
